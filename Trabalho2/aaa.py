...
def split_blocks(message, block_size=16, require_padding=True):
        ...
        retorno = [message[i:i+16] for i in range(0, len(message), block_size)]
...
class AES:
    ...
    def encrypt_ctr(self, plaintext, iv):
        ...
        for plaintext_block in split_blocks(plaintext, require_padding=False):
            ...
            aa =  self.encrypt_block(nonce)
          
            block = xor_bytes(plaintext_block, aa)
            blocks.append(block)
            nonce = inc_bytes(nonce)

        return b''.join(blocks)

    def encrypt_normal(self, plaintext):
        assert len(iv) == 16
    ...
    def decrypt_normal(self, plaintext):
        assert len(iv) == 16
...
if __name__ == '__main__':
    ...
    encrypted = AES(key).encrypt_normal(b'3243f6a8885a308d313198a2e0370734')
    print(AES(encrypted).decrypt_normal(iv))
